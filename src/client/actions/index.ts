...
export const fetchResume: ThunkActionCreator<any> = () => (
  async (dispatch, getState, api) =>
    dispatcher<Portfolio[]>(dispatch, FETCH_RESUME)(api.get(`/resume`))
);

export const fetchPortfolioItems: ThunkActionCreator<Portfolio[]> = (params: FetchPortfolioItemsParams) => (
  async (dispatch, getState, api) =>
    dispatcher<Portfolio[]>(dispatch, FETCH_PORTFOLIO_ITEMS)(api.get(`/portfolio?pageSize=${params.pageSize}`))
);
